...
For example to get the current weather in Bristol we uses this URL where `q` our query equals Bristol
...
It's making a call to the OpenWeatherMap API, don't forget to use your key and not mine!

Have a look in the console to see the JSON data.  In the ```draw()``` function of our p5 sketch we're taking the wind speed data and using it to scale a cirle, so far not a very interesting visualisation! 
...
If you need some help have a look at this p5 [example](https://p5js.org/examples/hello-p5-weather.html).  See how they have an ellipse moving around the screen in the direction of the wind.  This example also uses a weather vane but I'm sure it's not as good as yours!

### Extra challenge

* Add code from recent weeks so that your weather vane has particles that are attracted to it and are making sounds.
